...
export const Dashboard = ({
  ...
  return (
    <div className="w-full animate-in fade-in slide-in-from-bottom-4 space-y-6 duration-500">
      <section
        className={`overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br ${risk.panel} shadow-[0_8px_30px_rgb(0,0,0,0.06)] backdrop-blur-2xl`}
      >
        <div className="grid gap-0 lg:grid-cols-[320px_1fr]">
          <div className="border-b border-white/10 bg-[#0a0a0e]/40 p-8 backdrop-blur-md lg:border-b-0 lg:border-r">
            ...
            <div className="mt-5 text-center">
              <span
                className={`inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-sm font-semibold ${risk.soft}`}
              >
                {isHighRisk ? (
      ...
      <section
        className={`rounded-2xl border p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-2xl sm:p-8 ${
          derivedVerdict === "YES"
      ...
      >
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 text-center sm:text-left">
          ...
          <div className="w-full">
            <h2
              className={`text-xl font-bold mb-3 sm:mb-2 ${
                derivedVerdict === "YES"
                  ? "text-emerald-400"
                  : derivedVerdict === "CAUTION"
                    ? "text-yellow-400"
      ...
      <section className="rounded-2xl border border-white/10 bg-[#0a0a0e]/60 p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-2xl sm:p-8">
        ...
        <p className="max-w-5xl text-sm leading-7 text-slate-400 sm:text-base">
          ...
          <ol className="grid gap-3 lg:grid-cols-2">
            {(report.actionable_recommendations_for_bank || []).map(
              (rec, idx) => (
                ...
                >
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-emerald-500/20 text-xs font-bold text-emerald-400">
                    {idx + 1}
                  </span>
