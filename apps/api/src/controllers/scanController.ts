  ...
  try {
    ...
    if (botToken === process.env.BOT_API_TOKEN && whatsappSender) {
      let botUser = await prisma.user.findUnique({
        where: { email: "whatsapp-bot@system.local" },
      });
      if (!botUser) {
        botUser = await prisma.user.create({
          data: { email: "whatsapp-bot@system.local" },
        });
      }
      userId = botUser.id;
    }
    ...
    await scanQueue.add("analyze-apk", {
      ...
      filename,
      whatsappSender:
        typeof whatsappSender === "string" ? whatsappSender : undefined,
    });
...
export const getScanReport = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (scan.status !== "COMPLETED")
      return res
        .status(400)
        .json({ message: `Report not ready. Status: ${scan.status}` });
